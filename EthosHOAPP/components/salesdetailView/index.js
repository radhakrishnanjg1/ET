...
(function () {
    var view = app.salesdetailView = kendo.observable();
    var salesdetailViewModel = kendo.observable({
        onShow: function () {
            if (!app.utils.checkinternetconnection()) {
                return app.navigation.navigateoffline("salesdetailView");
            }
    ...
    view.set('salesdetailViewModel', salesdetailViewModel);
}());
...
function load_salesdetails_divisions(filterid) {
    ...
    var singledivision = JSON.parse(Enumerable.From(alldivision)
         .Where("$.SNO==" + filterid)
         .ToJSON());//.slice(0, 10)
    $("#span_sales_Division_Name").html(singledivision[0].Division_Name);
    $("#hdn_sales_SLNO").val(singledivision[0].SNO);
    $("#hdn_sales_Division_ID").val(singledivision[0].Division_ID);
}
...
function load_chart_financialyeardetails() {
    ...
    $("#chart-bar-financialyeardetails").kendoChart({
        theme: "nova",
        title: {
            text: "Month Wise Strategy Details",
...
function goto_sales_nextrecord(e) {
    ...
    if (newfilterid != filterid) {
        ...
        app.utils.loading(true);
        fun_db_APP_Get_Financial_Year_Sales_Details_By_Division_ID(division_id, filterid);
    }
} 
...
function fun_db_APP_Get_Financial_Year_Sales_Details_By_Division_ID(Division_ID, filterid) {
    var datasource = new kendo.data.DataSource({
        transport: {
            read: {
                url: "https://api.everlive.com/v1/wl2tdph1kbl8l9w8/Invoke/SqlProcedures/APP_Get_Financial_Year_Sales_Details_By_Division_ID",
                type: "POST",
                dataType: "json",
                data: {
